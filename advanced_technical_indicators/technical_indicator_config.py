...
class ConfigError(Exception):
    """Raised when an invalid technical indicator configuration is detected."""
    pass


@dataclass(frozen=True)
class TechnicalIndicatorProfile:
    name: str
    description: str
    language: str = "tr"
    ...
    allow_indicator_as_signal: bool = False
    ...
    enable_return_indicators: bool = True
    ...
    enable_channel_indicators: bool = True
    enable_candle_anatomy: bool = True
    enable_quote_microstructure: bool = True
    enable_mean_reversion: bool = True
    enable_warmup_nan_policy: bool = True
    enable_no_lookahead_guard: bool = True
    enable_validation_rules: bool = True
    enable_phase_118_handoff: bool = True
    min_readiness_score: float = 0.45
    enabled: bool = True
    notes: str = ""


TECHNICAL_INDICATOR_PROFILES: Dict[str, TechnicalIndicatorProfile] = {
    "balanced_local_technical_indicators": TechnicalIndicatorProfile(
        name="balanced_local_technical_indicators",
        description="Phase 117 offline/local teknik indikatör kütüphanesi için dengeli profil.",
        current_phase=117,
        ...
        notes="Dengeli local/offline teknik indikatör profili.",
    ),
    "strict_no_signal_indicator_safety": TechnicalIndicatorProfile(
        name="strict_no_signal_indicator_safety",
        description="İndikatörlerin sinyal/yönlü tahmin olarak kullanımını engelleyen sıkı profil.",
        current_phase=117,
        target_final_phase=160,
        next_phase=118,
        dry_run_default=True,
        ...
        notes="Sıkı güvenlik ve non-signal sınırları.",
    ),
    "dry_run_indicator_computation_focus": TechnicalIndicatorProfile(
        name="dry_run_indicator_computation_focus",
        description="Dry-run hesaplama provası ve şema doğrulamasına odaklanan profil.",
        current_phase=117,
        target_final_phase=160,
        next_phase=118,
        dry_run_default=True,
        ...
        research_only=True,
        min_readiness_score=0.45,
        notes="Dry-run prova odaklı profil.",
    ),
}


def get_technical_indicator_profile(name: str) -> TechnicalIndicatorProfile:
    if name not in TECHNICAL_INDICATOR_PROFILES:
        raise ConfigError(f"Unknown technical indicator profile: '{name}'. Available: {list(TECHNICAL_INDICATOR_PROFILES.keys())}")
    return TECHNICAL_INDICATOR_PROFILES[name]


def list_technical_indicator_profiles(enabled_only: bool = True) -> List[TechnicalIndicatorProfile]:
    if enabled_only:
        return [p for p in TECHNICAL_INDICATOR_PROFILES.values() if p.enabled]
    return list(TECHNICAL_INDICATOR_PROFILES.values())


def validate_technical_indicator_profiles() -> None:
    for name, p in TECHNICAL_INDICATOR_PROFILES.items():
        if p.current_phase != 117:
            raise ConfigError(f"Profile {name}: current_phase must be 117, got {p.current_phase}")
        if p.target_final_phase != 160:
            raise ConfigError(f"Profile {name}: target_final_phase must be 160, got {p.target_final_phase}")
        if p.next_phase != 118:
            raise ConfigError(f"Profile {name}: next_phase must be 118, got {p.next_phase}")
        ...
        forbidden_flags = [
            ...
            ("allow_indicator_as_signal", p.allow_indicator_as_signal),
...
def get_default_technical_indicator_profile() -> TechnicalIndicatorProfile:
    return get_technical_indicator_profile("balanced_local_technical_indicators")
