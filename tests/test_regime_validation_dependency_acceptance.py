"""Tests for Regime Validation Dependency Acceptance."""
...
def test_validation_dependency_acceptance():
    df, summary = build_regime_validation_dependency_acceptance_report()
    assert not df.empty
    assert summary["all_satisfied"] is True
    assert summary["satisfied_dependencies"] == len(df)

    s_df = summarize_regime_validation_dependency_acceptance(df)
