...
from advanced_provider_benchmark.news_metadata_provider_benchmark import (
    ...
    summarize_news_metadata_provider_benchmark,
)


def test_news_metadata_provider_benchmark():
    profile = get_default_provider_benchmark_profile()
    df, summary = build_news_metadata_provider_benchmark_report(profile)

    assert not df.empty
    assert summary["domain"] == "provider_domain_news_metadata"
