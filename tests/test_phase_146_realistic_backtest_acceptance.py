...
def test_phase_146_realistic_backtest_acceptance():
    df, summary = build_phase_146_realistic_backtest_acceptance_registry()
    assert isinstance(df, pd.DataFrame)
    assert isinstance(summary, dict)
    assert len(df) == len(PHASE_146_CHECKS)
    assert summary["phase_ref"] == "Phase 146"
