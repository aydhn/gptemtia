...
def test_phase_149_monte_carlo_acceptance():
    df, summary = build_phase_149_monte_carlo_acceptance_registry()
    assert isinstance(df, pd.DataFrame)
    assert isinstance(summary, dict)
    assert len(df) == len(PHASE_149_CHECKS)
    assert summary["phase_ref"] == "Phase 149"
