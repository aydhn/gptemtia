...
from advanced_benchmark_evaluation.risk_summary_placeholders import build_risk_summary_placeholder_registry

def test_build_risk_summary_placeholder_registry():
    profile = get_default_benchmark_evaluation_profile()
    df, summary = build_risk_summary_placeholder_registry(profile)
    assert not df.empty
    assert len(df) >= 2
