...
from advanced_provider_benchmark.fx_provider_benchmark import (
    ...
    summarize_fx_provider_benchmark,
)


def test_fx_provider_benchmark():
    profile = get_default_provider_benchmark_profile()
    df, summary = build_fx_provider_benchmark_report(profile)

    assert not df.empty
    assert summary["domain"] == "provider_domain_fx"
