"""Tests for Phase 134 Regime FeatureStore Config."""
...
from advanced_regime_featurestore_integration.regime_featurestore_config import (
    ...
    list_regime_featurestore_profiles,
...
def test_config_profiles_invariants():
    assert validate_regime_featurestore_profiles() is True

    profiles = list_regime_featurestore_profiles()
    assert len(profiles) >= 3

    for p in profiles:
        assert p.current_phase == 134
        assert p.target_final_phase == 160
        assert p.next_phase == 135
        ...
        assert p.allow_store_as_signal is False
...
def test_get_profile():
    default_p = get_default_regime_featurestore_profile()
    assert default_p.profile_name == "balanced_local_regime_featurestore_integration"

    strict_p = get_regime_featurestore_profile("strict_non_signal_regime_featurestore_safety")
    assert strict_p.min_readiness_score == 0.60

    with pytest.raises(ValueError):
        get_regime_featurestore_profile("non_existent_profile")
