# -*- coding: utf-8 -*-
"""Phase 150 Unit Tests: Data Snooping Bias Controls."""

from advanced_backtest_governance.backtest_governance_config import (
    get_default_backtest_governance_profile,
)
from advanced_backtest_governance.data_snooping_bias_controls import (
    build_data_snooping_bias_control_registry,
    DATA_SNOOPING_CONTROLS,
)


def test_build_data_snooping_bias_controls():
    profile = get_default_backtest_governance_profile()
    df, summary = build_data_snooping_bias_control_registry(profile)

    assert not df.empty
    assert len(df) == 3
    assert "control_id" in df.columns
    assert "name" in df.columns
    assert (df["execution_allowed"] == False).all()
    assert (df["status"] == "ACTIVE").all()
    assert summary["total_controls"] == 3
    assert len(DATA_SNOOPING_CONTROLS) == 3
