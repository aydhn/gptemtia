# -*- coding: utf-8 -*-
"""Unit tests for Phase 146: Backtest Benchmark Disabled Report."""
...
def test_build_benchmark_disabled_report():
    prof = get_default_realistic_backtest_profile()
    df, summary = build_backtest_benchmark_disabled_report(prof)
    assert not df.empty
    assert summary["is_blocked"] is True
    assert summary["policy_active"] is True
    assert summary["non_signal"] is True
    assert (df["status"].str.contains("DEFERRED_TO_PHASE_147")).all()
