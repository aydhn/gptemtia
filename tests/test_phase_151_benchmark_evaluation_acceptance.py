...
def test_phase_151_benchmark_evaluation_acceptance():
    df, summary = build_phase_151_benchmark_evaluation_acceptance_registry()
    assert isinstance(df, pd.DataFrame)
    assert isinstance(summary, dict)
    assert len(df) == len(PHASE_151_CHECKS)
    assert summary["phase_ref"] == "Phase 151"
