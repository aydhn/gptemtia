"""Tests for Cross-Asset Volatility Linkage."""

from advanced_cross_asset_regime_context.cross_asset_volatility_linkage import (
    VOLATILITY_LINKAGE_RECORDS,
    build_cross_asset_volatility_linkage_registry,
)


def test_build_cross_asset_volatility_linkage_registry():
    assert len(VOLATILITY_LINKAGE_RECORDS) >= 4
    df, summary = build_cross_asset_volatility_linkage_registry()
    assert len(df) >= 4
