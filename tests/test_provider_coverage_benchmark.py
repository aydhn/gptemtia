...
from advanced_provider_benchmark.provider_coverage_benchmark import (
    ...
    summarize_provider_coverage_benchmark,
)


def test_provider_coverage_benchmark():
    profile = get_default_provider_benchmark_profile()
    df, summary = build_provider_coverage_benchmark_report(profile)

    assert not df.empty
    assert "raw_score" in df.columns
    assert summary["mean_coverage_score"] > 0.0
