...
class NewsPublicDatasetProviderPlaceholder(BaseNewsMetadataProvider):
    ...
    def metadata(self) -> NewsProviderMetadata:
        ...
        return NewsProviderMetadata(
            ...
            "Public dataset news metadata provider placeholder", "https://example.org/dataset", "research",
    ...
    def fetch_news_metadata(self, request: NewsProviderRequest) -> NewsProviderResponse:
        return create_news_provider_response(
            ...
            output_ref="placeholder://public_dataset_news_metadata",
            row_count=0,
            schema_ref="news_metadata_schema_contract",
            warnings=["Public dataset placeholder; terms of use require manual verification."],
            manual_review_required=True
        )

    def health_check(self) -> Dict:
        return {"status": "placeholder_ready", "mode": "public-dataset-placeholder"}

def build_news_public_dataset_provider_placeholder(profile: NewsProviderProfile) -> Tuple[pd.DataFrame, Dict]:
    provider = NewsPublicDatasetProviderPlaceholder()
