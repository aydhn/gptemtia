...
class FXLicensedProviderPlaceholder(BaseFXProvider):
    provider_name = "fx_licensed_provider_placeholder"
    ...
    def metadata(self) -> FXProviderMetadata:
        return FXProviderMetadata(
            ...
            description="Placeholder for fx_licensed_provider_placeholder", homepage_ref="", license_note="requires manual review note",
...
def build_fx_licensed_provider_placeholder(profile: FXProviderProfile) -> Tuple[pd.DataFrame, Dict]:
    df = pd.DataFrame([{"provider": "fx_licensed_provider_placeholder", "status": "placeholder_active"}])
    return df, {"status": "ok"}
