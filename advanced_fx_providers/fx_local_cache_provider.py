...
class FXLocalCacheProviderPlaceholder(BaseFXProvider):
    provider_name = "fx_local_cache_provider_placeholder"
    ...
    def metadata(self) -> FXProviderMetadata:
        return FXProviderMetadata(
            ...
            description="Placeholder for fx_local_cache_provider_placeholder", homepage_ref="", license_note="",
...
def build_fx_local_cache_provider_placeholder(profile: FXProviderProfile) -> Tuple[pd.DataFrame, Dict]:
    df = pd.DataFrame([{"provider": "fx_local_cache_provider_placeholder", "status": "placeholder_active"}])
    return df, {"status": "ok"}
