...
class CalendarOfficialApiProviderPlaceholder(BaseCalendarProvider):
    provider_name = "calendar_official_api_provider_placeholder"
    provider_type = "provider_official_api"

    def metadata(self) -> CalendarProviderMetadata:
        return CalendarProviderMetadata(self.provider_name, self.provider_name, self.provider_type, "", "none", "none", "not stored/not printed/manual configuration only", "compliant", "none", "calendar_provider_ready", [])
...
def build_calendar_official_api_provider_placeholder(profile: CalendarProviderProfile) -> Tuple[pd.DataFrame, Dict]:
    df = pd.DataFrame([vars(CalendarOfficialApiProviderPlaceholder().metadata())])
