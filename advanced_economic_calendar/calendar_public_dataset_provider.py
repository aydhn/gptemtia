...
class CalendarPublicDatasetProviderPlaceholder(BaseCalendarProvider):
    provider_name = "calendar_public_dataset_provider_placeholder"
    provider_type = "provider_public_dataset"

    def metadata(self) -> CalendarProviderMetadata:
        return CalendarProviderMetadata(self.provider_name, self.provider_name, self.provider_type, "", "none", "manual review required", "none", "compliant", "none", "calendar_provider_ready", [])
...
def build_calendar_public_dataset_provider_placeholder(profile: CalendarProviderProfile) -> Tuple[pd.DataFrame, Dict]:
    df = pd.DataFrame([vars(CalendarPublicDatasetProviderPlaceholder().metadata())])
