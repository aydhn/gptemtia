...
class OfficialApiProviderPlaceholder(BaseDataProvider):
    ...
    def fetch(self, request: ProviderRequest) -> ProviderResponse:
        ...
        return create_provider_response(
            ...
            output_ref=f"official_api://{request.data_type}",
            row_count=0,
            manual_review_required=True
        )

    def health_check(self) -> dict:
        return {"status": "healthy", "mode": "official-api placeholder"}

def build_official_api_provider_placeholder(profile: DataProviderAbstractionProfile) -> tuple[pd.DataFrame, dict]:
    df = pd.DataFrame([{"provider": "official_api_provider_placeholder", "status": "placeholder_ready"}])
