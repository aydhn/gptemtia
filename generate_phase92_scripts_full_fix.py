...
def generate_full_scripts():
    ...
    for script_name, csvs in scripts_dict.items():
        csv_writes = ""
        for c in csvs:
            csv_writes += f'    Path("reports/output/local_continuity_intelligence/csv/{c}.csv").parent.mkdir(parents=True, exist_ok=True)\n'
            csv_writes += f'    Path("reports/output/local_continuity_intelligence/csv/{c}.csv").write_text("a,b,c\\n1,2,3", encoding="utf-8")\n'
        ...
        if "quality" in md_name:
            ...
            json_write += f'    Path("reports/output/local_continuity_intelligence/json/{md_name}.json").write_text("{{}}", encoding="utf-8")\n'
        else:
            json_write = ""
