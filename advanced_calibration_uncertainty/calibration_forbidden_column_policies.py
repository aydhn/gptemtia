# -*- coding: utf-8 -*-
"""Phase 141: Calibration Forbidden Column Policies."""
...
def build_calibration_forbidden_column_policy_registry(
    profile: Optional[CalibrationUncertaintyProfile] = None,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """Build DataFrame and summary for calibration forbidden column policies."""
    prof = profile or get_calibration_uncertainty_profile()
    rows = []
    for col in FORBIDDEN_COLUMNS_CALIBRATION:
        rows.append(
            {
    ...
    summary = summarize_calibration_forbidden_column_policies(df)
    return df, summary


def validate_calibration_forbidden_columns(column_names: List[str]) -> Dict[str, Any]:
    """Validate that provided column list contains zero forbidden columns."""
    violations = []
    for col in column_names:
        c_lower = col.lower()
        if c_lower in FORBIDDEN_COLUMNS_CALIBRATION or any(f == c_lower for f in FORBIDDEN_COLUMNS_CALIBRATION):
...
def summarize_calibration_forbidden_column_policies(df: pd.DataFrame) -> Dict[str, Any]:
    """Summarize calibration forbidden column policies DataFrame."""
    return {
