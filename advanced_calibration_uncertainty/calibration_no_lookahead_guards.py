# -*- coding: utf-8 -*-
"""Phase 141: Calibration No-Lookahead Guards."""
...
def build_calibration_no_lookahead_guard_registry(
    profile: Optional[CalibrationUncertaintyProfile] = None,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """Build DataFrame and summary for calibration no-lookahead guards."""
    prof = profile or get_calibration_uncertainty_profile()
    rows = [
        {
            "guard_id": "calib_guard_no_future_columns",
        ...
        {
            "guard_id": "calib_guard_temporal_asof_joins",
        ...
        {
            "guard_id": "calib_guard_monotonic_timestamps",
    ...
    summary = summarize_calibration_no_lookahead_guards(df)
    return df, summary


def validate_calibration_no_lookahead_columns(column_names: List[str]) -> Dict[str, Any]:
    """Validate column names against future lookahead terms."""
    violations = []
    for col in column_names:
        c_lower = col.lower()
        for term in LOOKAHEAD_TERMS:
            if term in c_lower:
...
def validate_no_future_calibration_join(
...
def summarize_calibration_no_lookahead_guards(df: pd.DataFrame) -> Dict[str, Any]:
    """Summarize calibration no-lookahead guards DataFrame."""
    return {
        "total_guards": len(df),
