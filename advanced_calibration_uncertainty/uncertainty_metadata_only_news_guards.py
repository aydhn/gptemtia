# -*- coding: utf-8 -*-
"""Phase 141: Uncertainty Metadata-Only News Guards."""
...
FORBIDDEN_NEWS_CONTENT_UNCERTAINTY = [
    "full_text",
    "article_body",
    "raw_content",
    "scraped_html",
...
def build_uncertainty_metadata_only_news_guard_registry(
    profile: Optional[CalibrationUncertaintyProfile] = None,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """Build DataFrame and summary for uncertainty metadata-only news guards."""
    prof = profile or get_calibration_uncertainty_profile()
    rows = [
        {
            "guard_id": "uncert_guard_no_full_text",
        ...
        {
            "guard_id": "uncert_guard_no_embeddings",
        ...
        {
            "guard_id": "uncert_guard_no_sentiment_scores",
    ...
    summary = summarize_uncertainty_metadata_only_news_guards(df)
    return df, summary


def validate_uncertainty_metadata_only_news_columns(column_names: List[str]) -> Dict[str, Any]:
    """Validate column names ensuring zero raw news content or embeddings."""
    violations = []
    for col in column_names:
        c_lower = col.lower()
        for term in FORBIDDEN_NEWS_CONTENT_UNCERTAINTY:
            if term in c_lower:
...
def summarize_uncertainty_metadata_only_news_guards(df: pd.DataFrame) -> Dict[str, Any]:
    """Summarize uncertainty metadata-only news guards DataFrame."""
    return {
        "total_guards": len(df),
