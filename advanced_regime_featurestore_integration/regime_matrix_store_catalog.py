"""Phase 134: Regime Matrix Store Catalog.

Connects Phase 127 multi-timeframe feature matrix contracts to the FeatureStore catalog.
"""
...
CANONICAL_MATRIX_ITEMS: List[Dict[str, Any]] = [
    {
        "store_catalog_name": "regime_matrix_daily_aligned_dataset",
        "store_entity_type": STORE_ENTITY_REGIME_MATRIX,
        "source_phase": 127,
        "source_component": "advanced_regime_matrix",
        "source_report_ref": "reports/output/advanced_regime_matrix/report_balanced_local_regime_matrix.json",
    ...
    {
        "store_catalog_name": "regime_matrix_weekly_aligned_dataset",
        "store_entity_type": STORE_ENTITY_REGIME_MATRIX,
        "source_phase": 127,
        "source_component": "advanced_regime_matrix",
        "source_report_ref": "reports/output/advanced_regime_matrix/report_balanced_local_regime_matrix.json",
    ...
    {
        "store_catalog_name": "regime_matrix_hourly_intraday_contract",
        "store_entity_type": STORE_ENTITY_REGIME_MATRIX,
        "source_phase": 127,
        "source_component": "advanced_regime_matrix",
        "source_report_ref": "reports/output/advanced_regime_matrix/report_balanced_local_regime_matrix.json",
...
def build_regime_matrix_store_catalog(
    profile: Optional[RegimeFeatureStoreProfile] = None,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """Construct DataFrame and summary for regime matrix store catalog."""
    active_profile = profile or get_regime_featurestore_profile()
    df = pd.DataFrame(CANONICAL_MATRIX_ITEMS)
    summary = {
        "domain": MATRIX_STORE_CATALOG_DOMAIN,
        "total_items": len(df),
...
def summarize_regime_matrix_store_catalog(df: pd.DataFrame) -> Dict[str, Any]:
    """Summarize regime matrix catalog DataFrame."""
    return {
        "total_items": len(df),
