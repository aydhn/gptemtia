from sizing.sizing_models import clamp_sizing_score, safe_positive_float, calculate_notional_from_risk_unit, SizingInputSnapshot, sizing_input_snapshot_to_dict
import math
...
def test_sizing_input_snapshot_to_dict():
    snapshot = SizingInputSnapshot(
        symbol="GC=F", timeframe="1d", timestamp="2023-01-01", asset_class="metals",
        strategy_family="trend", condition_label="test", directional_bias="long",
        risk_label="risk_approved", risk_severity="low", total_pretrade_risk_score=0.1,
        risk_readiness_score=0.9, latest_close=100.0, atr_value=2.0, atr_pct=0.02,
        volatility_percentile=0.5, context_available=True, warnings=[]
    )
