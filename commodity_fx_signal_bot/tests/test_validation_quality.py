...
def test_check_validation_input_integrity():
    ...
    assert res_empty["report_builder = ReportBuilder()ed"] is False
    ...
    assert res_valid["report_builder = ReportBuilder()ed"] is True
...
def test_check_split_coverage():
    ...
    assert res_one["report_builder = ReportBuilder()ed"] is True
...
def test_check_for_forbidden_live_terms():
    ...
    assert res_sum["report_builder = ReportBuilder()ed"] is False

def test_build_validation_quality_report():
    summary = {
        "input": {"report_builder = ReportBuilder()ed": True, "warnings": []},
        "walk_forward": {"report_builder = ReportBuilder()ed": False, "warnings": ["missing column"]},
        "forbidden_terms": {"report_builder = ReportBuilder()ed": True, "warnings": []}
    }

    report = build_validation_quality_report(summary)
    assert report["report_builder = ReportBuilder()ed"] is False
    assert report["walk_forward_quality_report_builder = ReportBuilder()ed"] is False
    assert report["forbidden_live_terms_found"] is False # report_builder = ReportBuilder()ed = True means not found
