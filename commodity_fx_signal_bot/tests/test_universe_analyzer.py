...
def test_analyze_symbol_success(settings, pipeline):
    ...
    df.attrs = {"requested_symbol": "TEST", "resolved_symbol": "TEST", "used_alias": False, "data_source": "yahoo"}
    pipeline.fetch_symbol_data.return_value = df

    analyzer = UniverseAnalyzer(pipeline, settings)
    spec = SymbolSpec(symbol="TEST", name="Test Symbol", asset_class="metals", sub_class="precious", currency="USD", data_source="yahoo")
...
def test_analyze_symbol_used_alias(settings, pipeline):
    ...
    df.attrs = {"requested_symbol": "TEST", "resolved_symbol": "TEST2", "used_alias": True, "data_source": "yahoo"}
    pipeline.fetch_symbol_data.return_value = df
...
def test_analyze_symbol_synthetic(settings, pipeline):
    analyzer = UniverseAnalyzer(pipeline, settings)
    spec = SymbolSpec(symbol="TEST", name="Test Symbol", asset_class="benchmark", sub_class="synthetic", currency="USD", data_source="synthetic")
...
def test_results_to_dataframe():
    res1 = SymbolReliabilityResult(
        symbol="TEST1", requested_symbol="TEST1", resolved_symbol="TEST1", name="Test1",
        asset_class="metals", sub_class="precious", data_source="yahoo", success=True,
        rows=100, start="2024-01-01", end="2024-04-10", last_close=10.0, missing_close_ratio=0.0,
        duplicate_index_count=0, negative_price_count=0, high_low_error_count=0, used_alias=False,
        error="", reliability_score=100.0, reliability_grade="A"
    )
