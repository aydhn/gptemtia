...
def test_normalize_ohlcv_basic(provider):
    ...
    # Check column names
    assert list(norm_df.columns) == ['open', 'high', 'low', 'close', 'adj_close', 'volume']
...
def test_normalize_ohlcv_missing_adj_close(provider):
    ...
    pd.testing.assert_series_equal(norm_df['close'], norm_df['adj_close'], check_names=False)
...
def test_normalize_ohlcv_missing_required_column(provider):
    ...
    with pytest.raises(DataProviderError, match="Missing required columns after normalization"):
