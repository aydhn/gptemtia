...
def test_should_block_sizing():
    ...
    risk_row = pd.Series({"risk_label": "risk_rejected_candidate", "risk_readiness_score": 0.1})
    eval_ctx = {"capped_theoretical_risk_amount": 100.0, "adjusted_theoretical_notional": 1000.0}
...
def test_should_watchlist_sizing():
    profile = get_default_sizing_profile()
    # Mock borderline
    risk_row = pd.Series({"risk_label": "risk_approved_candidate", "risk_readiness_score": profile.min_risk_readiness_score + 0.05})
    eval_ctx = {"capped_theoretical_risk_amount": 100.0, "adjusted_theoretical_notional": 1000.0}
...
def test_infer_sizing_candidate_label():
    profile = get_default_sizing_profile()
    # Good candidate
    risk_row = pd.Series({"risk_label": "risk_approved_candidate", "risk_readiness_score": 0.9, "total_pretrade_risk_score": 0.1})
    eval_ctx = {"capped_theoretical_risk_amount": 100.0, "adjusted_theoretical_notional": 1000.0, "exposure_eval": {"symbol_limit_passed": True, "asset_class_limit_passed": True}}
...
def test_rank_sizing_candidates():
    df = pd.DataFrame({
        "sizing_id": ["a", "b", "c"],
        "sizing_readiness_score": [0.5, 0.9, 0.2]
    })
