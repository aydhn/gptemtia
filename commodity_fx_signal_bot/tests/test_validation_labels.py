...
def test_list_validation_status_labels():
    ...
    assert len(labels) > 0
    assert "validation_report_builder = ReportBuilder()ed" in labels

def test_validate_validation_status():
    validate_validation_status("validation_report_builder = ReportBuilder()ed")
...
def test_validate_optimizer_candidate_label():
    validate_optimizer_candidate_label("optimizer_candidate_report_builder = ReportBuilder()ed")
    with pytest.raises(ValueError):
...
def test_blocking_status():
    ...
    assert is_blocking_validation_status("validation_report_builder = ReportBuilder()ed") is False
