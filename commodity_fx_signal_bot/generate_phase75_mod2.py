...
"""
with open(ls_dir / "phase_family_registry.py", "w", encoding="utf-8") as f: f.write(content_phase_family)

def generate_index_module(name, doc_flag=False, script_flag=False):
...
def classify_{name}_family(path: Path, project_root: Path) -> str:
    return "unknown_family"

{"def classify_artifact_status(path: Path, profile: LocalSynthesisProfile) -> str: return 'unknown'" if name == 'artifact' else ""}

{"def detect_forbidden_script_names(index_df: pd.DataFrame) -> pd.DataFrame: return index_df" if script_flag else ""}

def build_master_{name}_index(project_root: Path, {'family_df: pd.DataFrame, ' if name=='artifact' else ''}profile: LocalSynthesisProfile) -> Tuple[pd.DataFrame, Dict]:
    ...
    return df, summarize_master_{name}_index(df)

def summarize_master_{name}_index(index_df: pd.DataFrame) -> Dict:
    return {{"count": len(index_df), "note": "Not official audit inventory"}}
"""

for idx in ['artifact', 'report', 'datalake', 'docs', 'script', 'test']:
    with open(ls_dir / f"master_{idx}_index.py", "w", encoding="utf-8") as f:
        f.write(generate_index_module(idx, doc_flag=(idx=='docs'), script_flag=(idx=='script')))
