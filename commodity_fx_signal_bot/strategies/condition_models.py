...
def validate_rule_condition(condition: RuleCondition) -> None:
    if condition.operator not in _SUPPORTED_OPERATORS:
        raise ValueError(
            f"Unsupported operator '{condition.operator}' in rule condition '{condition.name}'"
        )
    if condition.operator == "between":
        if not isinstance(condition.right, (tuple, list)) or len(condition.right) != 2:
            raise ValueError(
                f"Operator 'between' requires 'right' to be a tuple/list of length 2 in '{condition.name}'"
            )
...
def validate_rule_template(template: RuleTemplate) -> None:
    ...
    if template.min_required_conditions < 0:
        raise ValueError(
            f"min_required_conditions must be >= 0 in template '{template.rule_id}'"
        )
