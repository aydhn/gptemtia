...
def main():
    ...
    parser.add_argument(
        "--top", type=int, default=20, help="Number of top candidates to show"
    )
    ...
    if not args.rebuild:
        try:
            if hasattr(data_lake, "load_sizing_pool"):
                ...
                logger.warning(
                    "load_sizing_pool not available on DataLake. Falling back to rebuild."
                )
    ...
    if u_df.empty or args.rebuild:
        logger.info(
            f"Building sizing pool for {args.timeframe} with profile {args.profile}"
        )
    ...
    report_text = build_sizing_pool_preview_report(
    ...
    report_path = (
        ...
        / f"sizing_pool_preview_{args.timeframe}_{args.profile}.txt"
    )
