...
def main():
    parser = argparse.ArgumentParser(
        description="Preview Sizing Candidates for a Symbol"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save", action="store_true", help="Save the generated candidates"
    )
    ...
    logger.info(
        f"Running sizing candidate build for {args.symbol} ({args.timeframe}) with profile {args.profile}"
    )

    df, summary = pipeline.build_for_symbol_timeframe(
        spec, args.timeframe, save=args.save
    )
    ...
    report_text = build_sizing_candidate_preview_report(
    ...
    report_path = (
        ...
        / f"sizing_candidate_preview_{args.symbol}_{args.timeframe}_{args.profile}.txt"
    )
