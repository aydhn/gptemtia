#!/usr/bin/env python
"""
Script to run a parameter sensitivity preview for a specific symbol.
"""
...
from reports.report_builder import build_parameter_sensitivity_preview_report

logging.basicConfig(level=getattr(logging, settings.log_level), format="%(asctime)s - %(name)s - %(levelname)s - %(message)s")
logger = logging.getLogger(__name__)

def main():
    parser = argparse.ArgumentParser(description="Run parameter sensitivity preview.")
    ...
    report_text = build_parameter_sensitivity_preview_report(args.symbol, args.timeframe, args.profile, summary, df)
    print("\n" + report_text)

    report_path = REPORTS_VALIDATION_DIR / f"parameter_sensitivity_preview_{args.symbol}_{args.timeframe}_{args.profile}.txt"
