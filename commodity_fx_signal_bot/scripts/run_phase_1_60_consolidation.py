#!/usr/bin/env python
"""
Run Phase 1-60 Consolidation script.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate Phase 1-60 Consolidation Matrix and Digest.")
    ...
    logger.info("Initializing Phase 1-60 Consolidation")
    settings = Settings()
    ...
    try:
        matrix_df, summary = pipeline.build_phase_1_60_consolidation(save=not args.no_save)
        logger.info(f"Phase 1-60 Consolidation generation complete. Summary: {summary}")
    except Exception as e:
        logger.error(f"Failed to generate phase consolidation: {e}")
        sys.exit(1)
