...
def main():
    parser = argparse.ArgumentParser(description="Simplification quality report")
    ...
    try:
        ...
        logger.info("Running Simplification quality report...")
        pipeline.build_simplification_quality_report(save=not args.no_save)

        logger.info(f"Successfully finished Simplification quality report")

    except Exception as e:
        logger.error(f"Error running Simplification quality report: {e}")
        traceback.print_exc()
        sys.exit(1)
