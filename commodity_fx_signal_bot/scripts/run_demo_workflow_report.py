"""
Script to generate the demo workflow packs.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate demo workflow reports")
    ...
    logger.info("Starting demo workflow generation...")
    ...
    df, summary = pipeline.build_demo_workflow_report(save=save)

    if save:
        # Generate and save txt report
        from reports.report_builder import build_demo_workflow_text_report
        from config.paths import REPORTS_SCENARIOS_TXT_DIR
        txt_report = build_demo_workflow_text_report(summary, df)
        REPORTS_SCENARIOS_TXT_DIR.mkdir(parents=True, exist_ok=True)
        with open(REPORTS_SCENARIOS_TXT_DIR / "demo_workflow_report.txt", "w") as f:
            f.write(txt_report)

    logger.info(f"Done. Generated {summary.get('total_packs')} workflows.")
