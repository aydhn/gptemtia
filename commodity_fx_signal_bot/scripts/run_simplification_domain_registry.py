...
def main():
    parser = argparse.ArgumentParser(description="Simplification domain registry")
    ...
    try:
        ...
        logger.info("Running Simplification domain registry...")
        pipeline.build_simplification_domain_registry(save=not args.no_save)

        logger.info(f"Successfully finished Simplification domain registry")

    except Exception as e:
        logger.error(f"Error running Simplification domain registry: {e}")
        traceback.print_exc()
        sys.exit(1)
