...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Preview volatility features for a specific symbol."
    )
    parser.add_argument(
        "--symbol",
        type=str,
        required=True,
        help="Symbol to build features for (e.g., GC=F)",
    )
    parser.add_argument(
        "--timeframe", type=str, default="1d", help="Timeframe (e.g., 1d, 4h)"
    )
    parser.add_argument(
        "--last", type=int, default=10, help="Number of latest rows to show"
    )
    parser.add_argument(
        "--full",
        action="store_true",
        help="Build full volatility feature set instead of compact",
    )
...
def main():
    ...
    if compact:
        features, summary = builder.build_compact_volatility_features(
            df, include_events=include_events
        )
    else:
        features, summary = builder.build_volatility_features(
            df, include_events=include_events
        )
    ...
    report = build_volatility_feature_preview_report(
    ...
    report_file = (
        ...
        / f"volatility_feature_preview_{args.symbol}_{args.timeframe}.txt"
    )
