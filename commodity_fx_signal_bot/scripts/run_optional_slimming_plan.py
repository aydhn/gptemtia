...
def main():
    parser = argparse.ArgumentParser(description="Optional slimming plan")
    parser.add_argument("--profile", type=str, default="slimming_plan_focus", help="Local Simplification Profile name")
    ...
    try:
        ...
        logger.info("Running Optional slimming plan...")
        pipeline.build_optional_slimming_plan(save=not args.no_save)

        logger.info(f"Successfully finished Optional slimming plan")

    except Exception as e:
        logger.error(f"Error running Optional slimming plan: {e}")
        traceback.print_exc()
        sys.exit(1)
