...
from artifact_metadata.metadata_report_builder import build_experiment_reproducibility_cards_markdown_report
...
def main():
    ...
    print(f"Total experiment cards: {summary.get('total_experiment_cards', 0)}")
    ...
    if args.save:
        ...
        for name, df in tables.items():
            if not df.empty:
                df.to_csv(REPORTS_ARTIFACT_METADATA_CSV_DIR / f"{name}{'_registry' if 'checklist' not in name else ''}.csv", index=False)

        md = build_experiment_reproducibility_cards_markdown_report(summary)
        with open(REPORTS_ARTIFACT_METADATA_MD_DIR / "experiment_reproducibility_cards_report.md", "w") as f:
             f.write(md)

        with open(REPORTS_ARTIFACT_METADATA_TXT_DIR / "experiment_reproducibility_cards_report.txt", "w") as f:
             f.write(md)
