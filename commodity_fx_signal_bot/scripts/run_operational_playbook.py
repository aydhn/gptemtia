#!/usr/bin/env python
"""
Run Operational Playbook script.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate Operational Playbook.")
    ...
    logger.info("Initializing Operational Playbook Generation")
    settings = Settings()
    ...
    try:
        playbook_text, summary = pipeline.build_operational_playbook(save=not args.no_save)
        logger.info(f"Operational Playbook generation complete. Summary: {summary}")
    except Exception as e:
        logger.error(f"Failed to generate operational playbook: {e}")
        sys.exit(1)
