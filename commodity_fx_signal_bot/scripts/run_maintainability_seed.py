...
def main():
    parser = argparse.ArgumentParser(description="Maintainability seed")
    ...
    try:
        ...
        logger.info("Running Maintainability seed...")
        pipeline.build_maintainability_seed(save=not args.no_save)

        logger.info(f"Successfully finished Maintainability seed")

    except Exception as e:
        logger.error(f"Error running Maintainability seed: {e}")
        traceback.print_exc()
        sys.exit(1)
