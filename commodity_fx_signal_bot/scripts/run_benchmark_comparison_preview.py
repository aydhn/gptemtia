...
def main():
    parser = argparse.ArgumentParser(
        description="Run benchmark comparison for a single symbol."
    )
    ...
    logger.info(
        f"Starting benchmark comparison preview for {args.symbol} ({args.timeframe})"
    )
    ...
    if hasattr(data_lake, "load_macro_features"):
        benchmark_df = data_lake.load_macro_features(args.timeframe)
    else:
        benchmark_df = pd.DataFrame()

    aligned_df, summary = build_benchmark_comparison_table(equity_curve, benchmark_df)

    if not summary:
        logger.warning("No benchmark data available or unable to align.")

    if hasattr(builder, "build_benchmark_comparison_preview_report"):
        report = builder.build_benchmark_comparison_preview_report(
            args.symbol, args.timeframe, args.backtest_profile, summary, aligned_df
        )
    else:
    ...
    if args.save:
        ...
        out_file = (
            out_dir
            / f"benchmark_comparison_preview_{args.symbol}_{args.timeframe}_{args.backtest_profile}.txt"
        )
