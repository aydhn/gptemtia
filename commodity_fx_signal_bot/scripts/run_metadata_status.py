...
# from config.paths import REPORTS_ARTIFACT_METADATA_CSV_DIR
...
from artifact_metadata.metadata_report_builder import build_metadata_status_markdown_report
...
def main():
    ...
    if args.save:
        ...
        pd.DataFrame([{"status": "ok"}]).to_csv(REPORTS_ARTIFACT_METADATA_CSV_DIR / "metadata_status.csv", index=False)

        md = build_metadata_status_markdown_report(summary)

        with open(REPORTS_ARTIFACT_METADATA_TXT_DIR / "metadata_status_report.txt", "w") as f:
             f.write(md)
