...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Report Summary Registry")
    parser.add_argument("--profile", type=str, default="balanced_local_summaries", help="Report summary profile name")
...
def main():
    ...
    data_lake = DataLake(root_dir=Path('/tmp')) # Mocked or proper setup
    pipeline = ReportSummarizationPipeline(data_lake, settings, PROJECT_ROOT, profile)

    tables, summary = pipeline.build_report_summary_registry(save=args.save)
    logger.info(f"Report Summary Registry complete. Found {len(tables.get('inventory', []))} reports.")
