...
def main():
    parser = argparse.ArgumentParser(description="Generate analyst task board from gaps and reports.")
    ...
    print(f"Generating analyst task board using profile: {profile.name}...")
    df, summary = pipeline.build_analyst_task_board(save=save_flag)

    print("\n--- Analyst Task Board Report ---")
    print(build_analyst_task_board_text_report(summary, df))
    print(f"\nDone. Found {summary.get('count', 0)} tasks.")
