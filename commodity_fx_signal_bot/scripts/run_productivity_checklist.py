...
def main():
    parser = argparse.ArgumentParser(description="Evaluate analyst productivity checklist.")
    ...
    print(f"Evaluating productivity checklist using profile: {profile.name}...")
    df, summary = pipeline.build_productivity_checklist(save=save_flag)

    print("\n--- Productivity Checklist Report ---")
    print(build_productivity_checklist_text_report(summary, df))
    print(f"\nDone. Passed items: {summary.get('passed_items', 0)} / {summary.get('total_items', 0)}")
