...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run paper portfolio preview for a symbol")
    ...
    parser.add_argument("--last", type=int, default=50, help="Number of last snapshots to show")
    ...
    if args.rebuild:
        ...
        portfolio_df = artifacts.get("portfolio", pd.DataFrame())
    else:
        print(f"Loading paper portfolio for {args.symbol} ({args.timeframe}) using profile {args.profile}...")
        try:
            portfolio_df = lake.load_paper_equity_curve(args.symbol, args.timeframe, args.profile)
            summary = lake.load_paper_summary(args.symbol, args.timeframe, args.profile)
        except Exception as e:
            print(f"Failed to load: {e}. Try running with --rebuild")
            return

    tail_df = portfolio_df.tail(args.last) if not portfolio_df.empty else None

    report_text = report_builder.build_paper_portfolio_preview_report(args.symbol, args.timeframe, args.profile, summary, tail_df)

    out_dir = Path("reports/output/paper_reports")
    out_dir.mkdir(parents=True, exist_ok=True)
    out_file = out_dir / f"paper_portfolio_preview_{args.symbol}_{args.timeframe}_{args.profile}.txt"
