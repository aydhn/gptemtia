"""
Script to build and save artifact evolution timeline.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run Artifact Evolution Timeline")
    ...
    try:
        ...
        logger.info("Building artifact evolution timeline...")
        tables, summary = pipeline.build_artifact_evolution_timeline(save=not args.no_save)

        logger.info(f"Successfully processed {summary.get('total_artifacts', 0)} artifacts.")

    except Exception as e:
        logger.error(f"Error building artifact evolution timeline: {e}")
        traceback.print_exc()
        sys.exit(1)
