...
def main():
    parser = argparse.ArgumentParser(description="Repo ergonomics rehearsal")
    ...
    try:
        ...
        logger.info("Running Repo ergonomics rehearsal...")
        pipeline.build_repo_ergonomics_rehearsal(save=not args.no_save)

        logger.info(f"Successfully finished Repo ergonomics rehearsal")

    except Exception as e:
        logger.error(f"Error running Repo ergonomics rehearsal: {e}")
        traceback.print_exc()
        sys.exit(1)
