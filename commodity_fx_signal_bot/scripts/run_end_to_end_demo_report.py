"""
Script to generate the end-to-end demo plan report.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate end-to-end demo report")
    ...
    logger.info("Starting E2E demo plan generation...")
    ...
    dfs, summary = pipeline.build_end_to_end_demo_report(save=save)
    plan_df = dfs.get("plan", None)

    if save:
        # Generate and save txt report
        from reports.report_builder import build_end_to_end_demo_text_report
        from config.paths import REPORTS_SCENARIOS_TXT_DIR
        txt_report = build_end_to_end_demo_text_report(summary, plan_df)
        REPORTS_SCENARIOS_TXT_DIR.mkdir(parents=True, exist_ok=True)
        with open(REPORTS_SCENARIOS_TXT_DIR / "end_to_end_demo_report.txt", "w") as f:
            f.write(txt_report)

    logger.info(f"Done. E2E demo plan contains {summary.get('total_steps')} steps.")
