...
from reports.report_builder import build_full_research_workflow_report
...
def main():
    ...
    try:
        ...
        profile = get_orchestration_profile("full_research_orchestration")
        ...
        manifest, summary = orchestrator.run_workflow("full_research_workflow", symbols, args.timeframe, dry_run=dry_run, save=args.save)

        df = pd.DataFrame(manifest.results) if manifest.results else None

        report_text = build_full_research_workflow_report(summary, df)

        if args.save:
            from config.paths import ORCHESTRATION_REPORT_OUTPUT_DIR
            ORCHESTRATION_REPORT_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
            txt_path = ORCHESTRATION_REPORT_OUTPUT_DIR / "full_research_workflow_report.txt"
        ...
        logger.error(f"Error running full research workflow: {e}", exc_info=True)
        sys.exit(1)
