...
def main():
    parser = argparse.ArgumentParser(description="Generate Research Debt Report")
    ...
    print(f"Starting Research Debt Report for timeframe {args.timeframe} and profile {args.profile}...")
    pipeline = ResearchPlanningPipeline(data_lake, settings, project_root, profile)

    df, summary = pipeline.build_research_debt_report(args.timeframe, args.save)

    print(f"Total debt items: {summary.get('total_debt_items', 0)}")
    print("WARNING: Research Debt indicates maintenance risks, NOT trade alarms or portfolio risks.")
