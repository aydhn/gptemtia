#!/usr/bin/env python3
"""
Script to run the artifact integrity check and generate observability reports.
"""
...
from reports.report_builder import build_artifact_integrity_report
...
def main():
    parser = argparse.ArgumentParser(description="Run Artifact Integrity Check.")
    ...
    logger.info("Running Artifact Integrity Check...")
    df, summary = pipeline.run_artifact_integrity_check(save=args.save)

    logger.info(f"Integrity Overall Status: {summary.get('status', 'unknown')}")
    logger.info(f"Invalid Artifacts: {summary.get('invalid_count', 0)}")

    if args.save:
        report_text = build_artifact_integrity_report(df, summary)
        out_dir = paths.REPORTS_OBSERVABILITY_DIR
        out_dir.mkdir(parents=True, exist_ok=True)

        txt_path = out_dir / "artifact_integrity_report.txt"
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write(report_text)

        csv_path = out_dir / "artifact_integrity.csv"
