#!/usr/bin/env python
"""
Run Master Operational Status Report script.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate Master Operational Status Report.")
    ...
    logger.info("Initializing Master Operational Status")
    settings = Settings()
    ...
    try:
        status_df, summary = pipeline.build_master_operational_status(save=not args.no_save)
        logger.info(f"Master Operational Status generation complete. Summary: {summary}")
    except Exception as e:
        logger.error(f"Failed to generate master operational status: {e}")
        sys.exit(1)
