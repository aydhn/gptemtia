...
def main():
    parser = argparse.ArgumentParser(description="Final modular complexity map")
    parser.add_argument("--profile", type=str, default="complexity_map_focus", help="Local Simplification Profile name")
    ...
    try:
        ...
        logger.info("Running Final modular complexity map...")
        pipeline.build_final_modular_complexity_map(save=not args.no_save)

        logger.info(f"Successfully finished Final modular complexity map")

    except Exception as e:
        logger.error(f"Error running Final modular complexity map: {e}")
        traceback.print_exc()
        sys.exit(1)
