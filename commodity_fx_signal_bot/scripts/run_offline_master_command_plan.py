#!/usr/bin/env python
"""
Run Offline Master Command Plan script.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate Offline Master Command Plan.")
    parser.add_argument("--profile", type=str, default="operator_master_plan", help="Master orchestration profile to use")
    parser.add_argument("--no-save", action="store_true", help="Do not save reports to disk")
    args = parser.parse_args()

    logger.info("Initializing Offline Master Command Plan Generation")
    settings = Settings()
    ...
    try:
        plan_df, summary = pipeline.build_offline_master_command_plan(save=not args.no_save)
        logger.info(f"Offline Master Command Plan generation complete. Summary: {summary}")
    except Exception as e:
        logger.error(f"Failed to generate offline master command plan: {e}")
        sys.exit(1)
