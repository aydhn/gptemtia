"""
Script to build and save the phase chronology report.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run Phase Chronology Report")
    ...
    try:
        ...
        logger.info("Building phase chronology report...")
        tables, summary = pipeline.build_phase_chronology_report(save=not args.no_save)

        logger.info(f"Successfully processed {summary.get('total_phases', 0)} phases.")

    except Exception as e:
        logger.error(f"Error building phase chronology report: {e}")
        traceback.print_exc()
        sys.exit(1)
