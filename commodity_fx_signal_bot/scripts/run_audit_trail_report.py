...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Audit Trail Report")
...
def main():
    ...
    df, summary = pipeline.build_audit_trail_report(save=args.save)


    txt_report = rb.build_audit_trail_text_report(summary, df)

    if args.save:
        txt_path = paths.REPORTS_GOVERNANCE_TXT_DIR / "audit_trail_report.txt"
        txt_path.parent.mkdir(parents=True, exist_ok=True)
        with open(txt_path, "w") as f:
            f.write(txt_report)

        csv_path = paths.REPORTS_GOVERNANCE_CSV_DIR / "audit_trail.csv"
        csv_path.parent.mkdir(parents=True, exist_ok=True)
        df.to_csv(csv_path, index=False)

        print(f"Audit trail report generated at {txt_path}")
    else:
        print(txt_report)
