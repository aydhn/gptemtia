...
def main():
    parser = argparse.ArgumentParser(description="Generate Research Backlog Report")
    ...
    print(f"Starting Research Backlog Report for timeframe {args.timeframe} and profile {args.profile}...")
    pipeline = ResearchPlanningPipeline(data_lake, settings, project_root, profile)

    df, summary = pipeline.build_research_backlog_report(args.timeframe, args.save)

    print(f"Total tasks generated: {summary.get('total_tasks_created', 0)}")
    print("WARNING: This output is an offline research planning backlog and is NOT for live trading or deploy execution.")
