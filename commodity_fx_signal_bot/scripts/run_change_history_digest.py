"""
Script to build and save change history digest.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run Change History Digest")
    ...
    try:
        ...
        logger.info("Building change history digest...")
        digest_txt, summary = pipeline.build_change_history_digest(save=not args.no_save)

        logger.info(f"Successfully generated digest with {summary.get('text_length', 0)} characters.")

    except Exception as e:
        logger.error(f"Error building change history digest: {e}")
        traceback.print_exc()
        sys.exit(1)
