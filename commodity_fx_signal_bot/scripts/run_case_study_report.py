"""
Script to generate the case studies report.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate synthetic case studies")
    ...
    logger.info("Starting case studies generation...")
    ...
    df, summary = pipeline.build_case_study_report(save=save)

    if save:
        # Generate and save txt report
        from reports.report_builder import build_case_study_text_report
        from config.paths import REPORTS_SCENARIOS_TXT_DIR
        txt_report = build_case_study_text_report(summary, df)
        REPORTS_SCENARIOS_TXT_DIR.mkdir(parents=True, exist_ok=True)
        with open(REPORTS_SCENARIOS_TXT_DIR / "case_study_report.txt", "w") as f:
            f.write(txt_report)

    logger.info(f"Done. Generated {summary.get('total')} case studies.")
