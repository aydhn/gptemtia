#!/usr/bin/env python3
"""
Orchestration pipeline workflow runner.
"""
import argparse
import logging
import sys
...
from reports.report_builder import build_pipeline_workflow_report

def main():
    parser = argparse.ArgumentParser(description="Run pipeline workflow.")
    parser.add_argument("--workflow", type=str, required=True, help="Workflow template name")
    ...
    parser.add_argument("--dry-run", action="store_true", default=True, help="Only plan and dry run")
    ...
    try:
        ...
        manifest, summary = orchestrator.run_workflow(args.workflow, symbols, args.timeframe, dry_run=dry_run, save=args.save)

        df = pd.DataFrame(manifest.results) if manifest.results else None

        report_text = build_pipeline_workflow_report(summary, df)

        if args.save:
            from config.paths import ORCHESTRATION_REPORT_OUTPUT_DIR
            ORCHESTRATION_REPORT_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
            txt_path = ORCHESTRATION_REPORT_OUTPUT_DIR / "pipeline_workflow_report.txt"
            with open(txt_path, "w") as f:
                f.write(report_text)

            if df is not None:
                csv_path = ORCHESTRATION_REPORT_OUTPUT_DIR / "pipeline_workflow_jobs.csv"
        ...
        logger.error(f"Error running pipeline workflow: {e}", exc_info=True)
        sys.exit(1)
