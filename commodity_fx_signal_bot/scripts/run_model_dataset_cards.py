...
from artifact_metadata.metadata_report_builder import build_model_dataset_cards_markdown_report
...
def main():
    ...
    print(f"Running model/dataset cards with profile: {profile.name}")
    tables, summary = pipeline.build_model_dataset_cards(save=False)

    print(f"Total model cards: {summary.get('total_model_cards', 0)}")
    ...
    if args.save:
        ...
        md = build_model_dataset_cards_markdown_report(summary)
        with open(REPORTS_ARTIFACT_METADATA_MD_DIR / "model_dataset_cards_report.md", "w") as f:
             f.write(md)

        with open(REPORTS_ARTIFACT_METADATA_TXT_DIR / "model_dataset_cards_report.txt", "w") as f:
             f.write(md)
