...
def main():
    parser = argparse.ArgumentParser(description="Send paper summary notification")
    parser.add_argument("--symbol", type=str, help="Specific symbol to report")
    parser.add_argument("--timeframe", type=str, default="1d", help="Timeframe (default: 1d)")
    parser.add_argument("--profile", type=str, default="balanced_telegram_reporting", help="Notification profile")
    ...
    logger.info("Starting paper summary script...")

    results, quality = pipeline.send_paper_summary(args.symbol, args.timeframe, save=True)
    ...
    report_text = report_builder.build_telegram_paper_summary_report(summary)

    report_path = paths.NOTIFICATION_REPORT_OUTPUT_DIR / "telegram_paper_summary_report.txt"
