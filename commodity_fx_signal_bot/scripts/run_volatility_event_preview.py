...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Preview volatility events for a symbol."
    )
    ...
    parser.add_argument(
        "--use-saved-features",
        action="store_true",
        help="Try loading pre-built features from lake",
    )
...
def main():
    ...
    if args.use_saved_features:
        try:
            ...
            if not features.empty:
                logger.info(
                    f"Loaded saved volatility features for {args.symbol} {args.timeframe}"
                )
    ...
    if features is None or features.empty:
        ...
        features, _ = builder.build_compact_volatility_features(
    ...
    report = build_volatility_event_preview_report(
        args.symbol, args.timeframe, summary, event_tail_df
    )

    VOLATILITY_REPORTS_DIR.mkdir(parents=True, exist_ok=True)
    report_file = (
        VOLATILITY_REPORTS_DIR
        / f"volatility_event_preview_{args.symbol}_{args.timeframe}.txt"
    )
