"""
Script to generate synthetic sample data for scenarios.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate synthetic sample data for scenarios")
    ...
    logger.info("Starting synthetic sample data generation...")
    ...
    df, summary = pipeline.build_sample_data_report(save=save)

    if save:
        # Generate and save txt report
        from reports.report_builder import build_sample_data_text_report
        from config.paths import REPORTS_SCENARIOS_TXT_DIR
        txt_report = build_sample_data_text_report(summary, df)
        REPORTS_SCENARIOS_TXT_DIR.mkdir(parents=True, exist_ok=True)
        with open(REPORTS_SCENARIOS_TXT_DIR / "sample_data_report.txt", "w") as f:
            f.write(txt_report)

    logger.info(f"Done. Sample data generated for {summary.get('files_saved', 0)} files.")
