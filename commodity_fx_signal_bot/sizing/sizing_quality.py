...
_FORBIDDEN_TRADE_TERMS = [
    "BUY", "SELL", "OPEN_LONG", "OPEN_SHORT", "CLOSE_POSITION",
    "MARKET_ORDER", "LIMIT_ORDER", "STOP_LOSS_ORDER", "TAKE_PROFIT_ORDER",
    "SEND_ORDER", "EXECUTE_TRADE", "REAL_POSITION_SIZE", "LIVE_POSITION",
    "BROKER_ORDER", "LEVERAGE_ORDER"
]
...
def check_sizing_score_ranges(df: pd.DataFrame) -> Dict[str, Any]:
    invalid_scores = 0
    cols = ["total_pretrade_risk_score", "risk_readiness_score", "sizing_readiness_score", "sizing_quality_score"]
...
def check_missing_sizing_fields(df: pd.DataFrame) -> Dict[str, Any]:
    required = ["sizing_id", "symbol", "timeframe", "sizing_label", "adjusted_theoretical_units"]
...
def build_sizing_quality_report(df: pd.DataFrame, summary: Dict[str, Any]) -> Dict[str, Any]:
    if df is None or df.empty:
    ...
    all_passed = (
        score_check["passed"] and
        dupe_check["passed"] and
        missing_check["passed"] and
    ...
    return {
        ...
        "warning_count": len(forbidden_check["forbidden_trade_terms_found"]) + len(missing_check["missing_required_fields"]),
        "passed": all_passed
    }
