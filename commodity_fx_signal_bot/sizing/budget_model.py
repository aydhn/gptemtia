...
def calculate_symbol_budget_remaining(existing_symbol_risk: float, max_symbol_risk_amount: float) -> float:
    """Calculates remaining budget for a symbol."""
    remaining = max_symbol_risk_amount - existing_symbol_risk
    return max(0.0, remaining)

def calculate_asset_class_budget_remaining(existing_asset_class_risk: float, max_asset_class_risk_amount: float) -> float:
    """Calculates remaining budget for an asset class."""
    remaining = max_asset_class_risk_amount - existing_asset_class_risk
    return max(0.0, remaining)

def calculate_total_budget_remaining(existing_total_risk: float, max_total_risk_amount: float) -> float:
