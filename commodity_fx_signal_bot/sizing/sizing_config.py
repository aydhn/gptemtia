...
def validate_sizing_profiles() -> None:
    ...
    for p in _DEFAULT_SIZING_PROFILES.values():
        if p.theoretical_account_equity <= 0:
            raise ConfigError(f"theoretical_account_equity must be positive for profile {p.name}")
        for attr in ["risk_per_candidate", "max_risk_per_symbol", "max_risk_per_asset_class", "max_total_portfolio_risk"]:
            val = getattr(p, attr)
            if not (0 <= val <= 1.0):
                raise ConfigError(f"{attr} must be between 0 and 1 for profile {p.name}, got {val}")
