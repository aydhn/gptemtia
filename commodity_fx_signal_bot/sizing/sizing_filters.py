...
def should_block_sizing(risk_row: pd.Series, sizing_eval: Dict[str, Any], profile: SizingProfile) -> Tuple[bool, List[str]]:
    ...
    readiness = safe_positive_float(sizing_eval.get("risk_readiness_score", risk_row.get("risk_readiness_score", 0.0)))
    if readiness is not None and readiness < profile.min_risk_readiness_score:
        reasons.append(f"Risk readiness score ({readiness}) is below minimum ({profile.min_risk_readiness_score}).")

    # Check total pretrade risk
    pretrade_risk = safe_positive_float(sizing_eval.get("total_pretrade_risk_score", risk_row.get("total_pretrade_risk_score", 0.0)))
    if pretrade_risk is not None and pretrade_risk > profile.max_total_pretrade_risk:
        reasons.append(f"Total pretrade risk ({pretrade_risk}) is above maximum ({profile.max_total_pretrade_risk}).")

    # Check budget constraints
    capped_amount = sizing_eval.get("capped_theoretical_risk_amount", sizing_eval.get("theoretical_risk_amount", 1.0))
    if capped_amount <= 0:
...
def should_watchlist_sizing(risk_row: pd.Series, sizing_eval: Dict[str, Any], profile: SizingProfile) -> Tuple[bool, List[str]]:
    ...
    readiness = safe_positive_float(sizing_eval.get("risk_readiness_score", risk_row.get("risk_readiness_score", 0.0)))
    if readiness is not None and profile.min_risk_readiness_score <= readiness < (profile.min_risk_readiness_score + 0.1):
        reasons.append("Risk readiness is borderline.")

    # Check pretrade risk borderline
    pretrade_risk = safe_positive_float(sizing_eval.get("total_pretrade_risk_score", risk_row.get("total_pretrade_risk_score", 0.0)))
    if pretrade_risk is not None and (profile.max_total_pretrade_risk - 0.1) < pretrade_risk <= profile.max_total_pretrade_risk:
        reasons.append("Total pretrade risk is borderline.")

    return len(reasons) > 0, reasons

def infer_sizing_candidate_label(risk_row: pd.Series, sizing_eval: Dict[str, Any], profile: SizingProfile) -> str:
...
def filter_sizing_candidates_by_readiness(df: pd.DataFrame, min_readiness: float) -> pd.DataFrame:
...
def rank_sizing_candidates(df: pd.DataFrame, top_n: Optional[int] = None) -> pd.DataFrame:
