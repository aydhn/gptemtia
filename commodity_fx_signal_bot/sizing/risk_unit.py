...
def calculate_risk_per_unit_from_atr(
    atr_value: Optional[float], atr_multiplier: float = 1.0
) -> Optional[float]:
...
def calculate_risk_per_unit_from_percent(
    price: Optional[float], risk_pct: float
) -> Optional[float]:
...
def calculate_theoretical_unit_count(
    risk_amount: float, risk_per_unit: Optional[float]
) -> float:
    """
...
def calculate_theoretical_notional(
    unit_count: float, price: Optional[float]
) -> Optional[float]:
...
def calculate_risk_unit_frame(
    ...
    if len(df) == 0:
        return out_df, {
            "warnings": ["Empty dataframe provided to risk unit frame calculator"]
        }
    ...
    atr_series = (
        df[atr_col] if atr_col in df.columns else pd.Series(index=df.index, dtype=float)
    )

    # ATR 1x
    out_df["risk_per_unit_atr_1x"] = atr_series
    out_df["theoretical_unit_count_atr_1x"] = (
        risk_amount / out_df["risk_per_unit_atr_1x"]
    ).replace([float("inf"), -float("inf")], float("nan"))
    out_df["theoretical_notional_atr_1x"] = (
        out_df["theoretical_unit_count_atr_1x"] * price_series
    )

    # ATR 2x
    out_df["risk_per_unit_atr_2x"] = atr_series * 2.0
    out_df["theoretical_unit_count_atr_2x"] = (
        risk_amount / out_df["risk_per_unit_atr_2x"]
    ).replace([float("inf"), -float("inf")], float("nan"))
    out_df["theoretical_notional_atr_2x"] = (
        out_df["theoretical_unit_count_atr_2x"] * price_series
    )
