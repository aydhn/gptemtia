...
def create_files():
    ...
    scripts = [
        "run_packaging_domain_registry.py",
        "run_distribution_bundle_rehearsal.py",
        "run_portable_docs_bundle.py",
        "run_offline_release_folder_manifest.py",
        "run_terminal_handover_zip_map.py",
        "run_final_packaging_governance.py",
        "run_packaging_quality_report.py",
        "run_packaging_status.py"
    ]

    for script in scripts:
        with open(base_dir / script, "w", encoding="utf-8") as f:
...
def main():
    ...
    print(f"Running {script} with profile: {{args.profile}}")
    settings = Settings()
