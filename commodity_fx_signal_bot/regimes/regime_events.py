...
def detect_regime_transition_events(
    regime_df: pd.DataFrame, config: RegimeEventConfig | None = None
) -> pd.DataFrame:
    ...
    events["event_regime_changed_recently"] = (
        is_transition.rolling(window=config.transition_lookback, min_periods=1)
        .max()
        .fillna(0)
        .astype(int)
    )
...
def detect_trend_regime_events(
    regime_df: pd.DataFrame, config: RegimeEventConfig | None = None
) -> pd.DataFrame:
    ...
    if "regime_primary_label" in regime_df.columns:
        ...
        events["event_strong_trend_regime"] = labels.isin(
...
def detect_range_regime_events(
    regime_df: pd.DataFrame, config: RegimeEventConfig | None = None
) -> pd.DataFrame:
    ...
    if "regime_primary_label" in regime_df.columns:
        ...
        events["event_compressed_range_regime"] = (labels == "compressed_range").astype(
            int
        )
...
def detect_volatility_regime_events(
    ...
    if "regime_secondary_label" in regime_df.columns:
        ...
        events["event_high_volatility_regime"] = (
            sec_labels == "high_volatility"
        ).astype(int)
        events["event_low_volatility_regime"] = (sec_labels == "low_volatility").astype(
            int
        )
        events["event_volatility_expansion_regime"] = (
            sec_labels == "volatility_expansion"
        ).astype(int)

    return events


def detect_conflict_regime_events(
    regime_df: pd.DataFrame, config: RegimeEventConfig | None = None
) -> pd.DataFrame:
    ...
    if "regime_primary_label" in regime_df.columns:
        events["event_conflicting_regime"] = (
            regime_df["regime_primary_label"] == "conflicting_regime"
        ).astype(int)

    if "regime_confidence" in regime_df.columns:
        events["event_insufficient_regime_confidence"] = (
            regime_df["regime_confidence"] < config.confidence_threshold
        ).astype(int)

    return events


def detect_mean_reversion_regime_events(
    regime_df: pd.DataFrame, config: RegimeEventConfig | None = None
) -> pd.DataFrame:
    ...
    if "regime_secondary_label" in regime_df.columns:
        events["event_mean_reversion_friendly_regime"] = (
            regime_df["regime_secondary_label"] == "mean_reversion_friendly"
        ).astype(int)

    return events


def build_regime_event_frame(
    regime_df: pd.DataFrame, config: RegimeEventConfig | None = None
