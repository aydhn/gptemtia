...
Measures alignment/conflict between ML context and decision candidates.
"""
...
def calculate_model_decision_alignment(
    decision_row: pd.Series,
    ml_row: Union[pd.Series, Dict, None],
    profile: MLIntegrationProfile,
) -> dict:
    """Calculate alignment between a decision candidate and ML context."""
    if ml_row is None or (isinstance(ml_row, pd.Series) and ml_row.empty) or pd.isna(ml_row.get("predicted_direction", pd.NA)):
        return {
            ...
            "model_decision_alignment_score": 0.5,
            "warnings": ["ML context is unavailable"],
        }

    candidate_type = str(decision_row.get("candidate_type", "no_trade")).lower()

    # Map decision candidate type to directional bias
    ...
    if uncertainty_penalty > 0 and profile.allow_uncertain_context_as_neutral:
        ...
        warnings.append("ML prediction conflicts with decision direction")
    elif support > 0.0:
    ...
    return {
        ...
        "model_decision_alignment_score": alignment_score,
        "warnings": warnings,
    }


def build_model_decision_alignment_frame(
    decision_df: pd.DataFrame,
    ml_context_df: pd.DataFrame,
    profile: MLIntegrationProfile,
) -> Tuple[pd.DataFrame, dict]:
    """Build a DataFrame containing decision alignment metrics."""
    summary = {"status": "success", "warnings": []}

    if decision_df is None or decision_df.empty:
        summary["status"] = "unavailable"
        summary["warnings"].append("Decision DataFrame is empty or None")
        return pd.DataFrame(), summary

    if ml_context_df is None or ml_context_df.empty:
        results = []
        for idx, row in decision_df.iterrows():
            results.append(calculate_model_decision_alignment(row, None, profile))
        return pd.DataFrame(results, index=decision_df.index), {"status": "unavailable", "warnings": ["ML context empty"]}

    # Reindex ML context to match decisions
    ml_context_df = ml_context_df.sort_index()
    aligned_ml = ml_context_df.reindex(decision_df.index, method="ffill")

    results = []
    for idx, row in decision_df.iterrows():
        ml_row = aligned_ml.loc[idx] if idx in aligned_ml.index else None
        results.append(calculate_model_decision_alignment(row, ml_row, profile))

    result_df = pd.DataFrame(results, index=decision_df.index)
