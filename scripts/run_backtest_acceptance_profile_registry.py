# -*- coding: utf-8 -*-
"""Phase 152: Run Backtest Acceptance Profile Registry Script.
...
from reports.report_builder import build_backtest_acceptance_text_report
...
def main():
    ...
    df_prof, s_prof = build_backtest_acceptance_profile_registry(profile)
    df_dom, s_dom = build_backtest_acceptance_domain_registry(profile)
    df_scp, s_scp = build_backtest_acceptance_scope_registry(profile)

    data_lake.save_backtest_acceptance_profile_registry(df_prof, s_prof)
    data_lake.save_backtest_acceptance_domain_registry(df_dom, s_dom)
    data_lake.save_backtest_acceptance_scope_registry(df_scp, s_scp)

    md_report = build_backtest_acceptance_profile_markdown_report(s_prof, df_prof)
    txt_report = build_backtest_acceptance_text_report(s_prof, df_prof)
    ...
    print("=" * 70)
    print("PHASE 152: BACKTEST ACCEPTANCE PROFILES & DOMAINS")
    print("=" * 70)
