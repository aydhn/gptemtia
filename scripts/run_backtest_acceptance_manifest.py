# -*- coding: utf-8 -*-
"""Phase 152: Run Backtest Acceptance Manifest Script.

Builds readiness score, master manifest, Phase 153 handoff report, and full markdown report.
"""
...
from reports.report_builder import (
    build_backtest_acceptance_readiness_score_text_report,
    build_backtest_acceptance_manifest_text_report,
    build_phase_153_handoff_text_report,
)
...
def main():
    ...
    df_scr, s_scr = build_backtest_acceptance_readiness_score_report(profile)
    df_mnf, s_mnf = build_backtest_acceptance_manifest(profile)
    df_hnd, s_hnd = build_phase_153_portfolio_construction_position_sizing_risk_budgeting_handoff_report(profile)
    df_cmp, s_cmp = build_backtest_acceptance_component_registry(profile)

    data_lake.save_backtest_acceptance_readiness_score_report(df_scr, s_scr)
    data_lake.save_backtest_acceptance_manifest(df_mnf, s_mnf)
    data_lake.save_phase_153_portfolio_construction_position_sizing_risk_budgeting_handoff_report(df_hnd, s_hnd)
    ...
    with open(out_dir / "readiness_score.md", "w", encoding="utf-8") as f:
        f.write(build_backtest_acceptance_readiness_score_markdown_report(s_scr, df_scr))
    with open(out_dir / "readiness_score.txt", "w", encoding="utf-8") as f:
        f.write(build_backtest_acceptance_readiness_score_text_report(s_scr, df_scr))

    with open(out_dir / "manifest.md", "w", encoding="utf-8") as f:
        f.write(build_backtest_acceptance_manifest_markdown_report(s_mnf, df_mnf))
    with open(out_dir / "manifest.txt", "w", encoding="utf-8") as f:
        f.write(build_backtest_acceptance_manifest_text_report(s_mnf, df_mnf))

    with open(out_dir / "phase_153_handoff.md", "w", encoding="utf-8") as f:
        f.write(build_phase_153_handoff_markdown_report(s_hnd, df_hnd))
    with open(out_dir / "phase_153_handoff.txt", "w", encoding="utf-8") as f:
        f.write(build_phase_153_handoff_text_report(s_hnd, df_hnd))
    ...
    with open(out_dir / "backtest_acceptance_report.md", "w", encoding="utf-8") as f:
        f.write(full_md)

    data_lake.save_backtest_acceptance_report(
    ...
    print("=" * 70)
    print("PHASE 152: BACKTEST ACCEPTANCE MANIFEST & READINESS SCORE")
    print("=" * 70)
    ...
    print(f"Backtest Block Done    : {s_mnf['backtest_block_completed']}")
    print(f"Phase 153 Handoff Ready: {s_hnd['handoff_ready']}")
